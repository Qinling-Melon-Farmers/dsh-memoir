...
export function validateMemoirSettingsPatch(payload: unknown, base: MemoirSettings = DEFAULT_MEMOIR_SETTINGS): string | undefined {
  ...
  for (const key of BOOLEAN_FIELDS) {
    if (key in record && typeof record[key] !== 'boolean') return `${key} must be a boolean`
  }
...
export class MemoirSettingsStore {
  ...
  private load(): void {
    ...
    try {
      ...
      if (parsed.version !== SETTINGS_VERSION && parsed.version !== LEGACY_SETTINGS_VERSION) return
