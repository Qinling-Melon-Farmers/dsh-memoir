...
import { tokenizeQuery, type RankedEntry, type RetrievalEngine } from './retrieval.js'
...
function combinedText(value: Pick<EntryPayload, 'title' | 'content'> | Pick<MemoirEntry, 'title' | 'content'>): string {
  return `${value.title ?? ''}\n${value.content}`.trim()
}
...
function classify(
  ...
  ranked: RankedEntry,
  bm25: number,
): Omit<SimilarityCandidate, 'entry' | 'projectPath'> | undefined {
  ...
  if (!duplicate && !conflict) return undefined

  const reasons: SimilarityReason[] = []
  if (exactContent) reasons.push('exact-content')
  if (exactTitle) reasons.push('exact-title')
  else if (title >= SIMILARITY_POLICY.conflictTitleSimilarity) reasons.push('high-title-overlap')
  if (tokens >= SIMILARITY_POLICY.conflictTokenJaccard) reasons.push('high-token-overlap')
  if (bm25 >= 0.5) reasons.push('bm25-candidate')
  if (conflict) reasons.push('same-topic-different-content')
...
export function findSimilarMemories(
  ...
  cwd: string,
  payload: EntryPayload,
  limit = SIMILARITY_POLICY.outputCandidates,
): SimilarityCandidate[] {
  const query = combinedText(payload).slice(0, 4_000)
  if (tokenizeQuery(query).length === 0) return []
