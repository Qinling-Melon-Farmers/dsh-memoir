...
export class MemorySnapshotStore implements SnapshotPersistence {
  ...
  private stats: SnapshotPersistenceDiagnostics = { restored: 0, created: 0, failures: 0, lastStatus: 'idle', lastError: null }
  ...
  private valid(snapshot: SessionSnapshot, key: string): boolean {
  ...
  private read(path: string, key: string, scope: string): SessionSnapshot | undefined {
    try {
      const info = lstatSync(path)
      if (!info.isFile() || info.isSymbolicLink()) throw new InvalidSnapshot('invalid-record')
    } catch (error) {
    ...
    let raw: string
    try {
      if (fstatSync(fd).size > MAX_SNAPSHOT_RECORD_BYTES) throw new InvalidSnapshot('record-too-large')
      ...
      if (size > MAX_SNAPSHOT_RECORD_BYTES) throw new InvalidSnapshot('record-too-large')
      raw = buffer.subarray(0, size).toString('utf8')
    } finally { closeSync(fd) }
    let record: { version?: unknown; scope?: unknown; snapshot?: SessionSnapshot }
    try { record = JSON.parse(raw) } catch { throw new InvalidSnapshot('invalid-record') }
  ...
  getOrCreate(key: string, builder: () => SessionSnapshot): SessionSnapshot {
    ...
    let built: SessionSnapshot | undefined
    let builderFailed = false
    const create = () => {
      if (built !== undefined) return built
      try { built = builder(); return built } catch (error) { builderFailed = true; throw error }
    }
    try {
      if (Buffer.byteLength(key, 'utf8') > 8192) throw new InvalidSnapshot('record-too-large')
      ...
      return withFileLock(path + '.lock', () => {
        ...
        if (!this.valid(snapshot, key)) throw new InvalidSnapshot('record-too-large')
        const raw = JSON.stringify({ version: SNAPSHOT_FORMAT_VERSION, scope, snapshot }) + '\n'
        if (Buffer.byteLength(raw, 'utf8') > MAX_SNAPSHOT_RECORD_BYTES) throw new InvalidSnapshot('record-too-large')
      ...
      if (!this.warned.has(code)) {
        ...
        try { this.warning?.(code) } catch { /* observer only */ }
      }
