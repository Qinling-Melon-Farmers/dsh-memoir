...
/** Minimal alpha session-list shape consumed by the native view adapters. */
interface SessionListSnapshotLike {
  current?: string
  byId: Record<string, { cwd?: string } | undefined>
}

/** Framework-bound selector Hook supplied through alpha global slot props. */
type UseSessions = <Selected>(
  selector: (snapshot: SessionListSnapshotLike) => Selected,
) => Selected

interface NativeGlobalProps {
  useSessions: UseSessions
}

interface NativeConversationViewProps extends NativeGlobalProps {
  sessionId: string
...
interface NativeSettingsSectionProps extends NativeGlobalProps {
  close(): void
}

interface NativeSlotOptions {
  name: string
  id?: string
  order?: number
  label?: string | (() => string)
...
interface NativeSlotsLike {
  inject(name: string, register: () => () => void): () => void
  register<Props>(options: NativeSlotOptions, component: (props: Props) => ReactNode): () => void
}

type AlphaClientContext = Omit<Context, 'sessions' | 'slots'> & {
  sessions: {
    open(sessionId: string): void
  }
  slots: NativeSlotsLike
}
...
function ConversationMemoirView({
  ...
  viewRequest,
  completeViewRequest,
}: NativeConversationViewProps & {
  api: MemoirApi
  ctx: AlphaClientContext
  t: (key: string) => string
}) {
...
export function apply(rawCtx: Context): void {
  const ctx = rawCtx as unknown as AlphaClientContext
  ...
  ctx.effect(() => {
    ...
    const register = (): void => {
      ...
      disposers = [
        ctx.slots.inject('conversation.view', () => ctx.slots.register<NativeConversationViewProps>({
          name: 'conversation.view',
          id: 'memoir',
          order: 20,
          label: () => t('entry.label'),
        }, props => <ConversationMemoirView {...props} api={api} ctx={ctx} t={t} />)),
        ctx.slots.inject('settings.section', () => ctx.slots.register<NativeSettingsSectionProps>({
