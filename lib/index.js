...
export function memoirGuidance(language = DEFAULT_MEMOIR_LANGUAGE) {
...
export function memoirSectionHeading(language = DEFAULT_MEMOIR_LANGUAGE) {
 ...
 */
export function memoirSectionText(store, context, manager, budget = DEFAULT_MEMORY_BUDGET, language = DEFAULT_MEMOIR_LANGUAGE) {
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => {
        ...
        const dispose = ctx.on('tools/result', (exec, result) => {
            if (exec.name !== 'memoir_record' && exec.name !== 'memoir_update')
                return;
            if (seen.has(exec.token))
                return;
            seen.add(exec.token);
            if (seen.size > 4096)
    ...
    ctx.effect(() => installAutoDistill(autoDistillWire(ctx), {
        ...
        activity: (agent) => ctx.sessionProjections.stateOf(agent.session, ACTIVITY_KEY),
