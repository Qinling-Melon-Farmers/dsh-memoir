...
export function makeRoutes(store, diagnostics, retrieval, hotMemory, allowedWorkspace, touchWorkspace) {
    const handler = async (req, res) => {
        ...
        if (method === 'POST') {
            ...
            const entry = store.record(path, recordPayload, sessionId);
            json(res, OK({ entry }));
            return;
        }
        ...
        if (method === 'PATCH') {
            ...
            if (id === null || (status !== undefined && (typeof status !== 'string' || !MEMOIR_STATUSES.includes(status))) || (pinned !== undefined && typeof pinned !== 'boolean')) {
                json(res, FAIL(BAD_REQUEST), 400);
                return;
            }
