...
function clampOutput(parts) {
    const text = parts.filter((p) => p !== '').join('\n\n');
    if (text.length <= READ_OUTPUT_MAX_CHARS)
        return text;
...
export function memoirRecordTool(store) {
    return defineTool({
        ...
        output: {
            ...
            render: (_args, value) => text('已记录 [' + value.section + '] ' + (value.title !== undefined && value.title !== '' ? value.title + ' ' : '') + '(id: ' + value.id + ')'),
        },
...
export function memoirReadTool(store, options) {
    const defaultLimit = options?.defaultLimit ?? 8;
    const maxLimit = options?.maxLimit ?? 30;
    return defineTool({
        name: 'memoir_read',
        description: '读取项目持久记忆与经验教训（默认返回最近 ' + defaultLimit + ' 条 compact 摘要）。开始新会话或接手旧项目时先调用。' +
                ...
                if (cwd === undefined) {
                    ...
                    if (matched.length === 0) {
                        ...
                        if (matched.length > entries.length)
                            parts.push(clippedNote(matched.length, entries.length));
                    }
                }
            }
