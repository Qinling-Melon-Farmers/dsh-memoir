...
function trackWrite(store, exec, hooks, mutate) {
 ...
 */
export function resolveMemorySource(exec, hooks) {
...
export function renderEntryFull(entry, language = DEFAULT_MEMOIR_LANGUAGE) {
...
function clippedNote(total, shown, language) {
...
function appendGrouped(budget, entries, renderEntry, language) {
    let shown = 0;
    let lastSection = '';
    for (const entry of entries) {
        if (entry.section !== lastSection) {
            lastSection = entry.section;
            if (!budget.add('## ' + sectionCopy(entry.section, language).label))
                break;
        }
...
export function memoirRecordTool(store, retrieval, languageSource = DEFAULT_MEMOIR_LANGUAGE, hooks) {
    const currentLanguage = () => languageFrom(languageSource);
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                if (value.action === 'needs-resolution') {
                    ...
                    return text(`${copy.needsResolution(value.candidates.length)}\n${candidates}\n${copy.resolutionInstruction}`);
                }
...
export function memoirUpdateTool(store, languageSource = DEFAULT_MEMOIR_LANGUAGE, hooks) {
    const currentLanguage = () => languageFrom(languageSource);
    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (entry === undefined)
                throw new Error(copy.notFound(args.id));
...
export function memoirReadTool(store, options, retrieval, languageSource = DEFAULT_MEMOIR_LANGUAGE) {
    const currentLanguage = () => languageFrom(languageSource);
    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            const renderEntry = detail === 'full'
                ? (entry) => renderEntryFull(entry, language)
                : renderEntryCompact;
                ...
                if (cwd === undefined) {
                    ...
                    if (ranked.length > 0) {
                        ...
                        if (ranked.length > entries.length)
                            budget.add(clippedNote(ranked.length, entries.length, language));
                    }
                    else {
                        ...
                        if (matched.length === 0) {
                            ...
                            if (matched.length > entries.length)
                                budget.add(clippedNote(matched.length, entries.length, language));
                        }
                    }
                }
            }
            if (scope === 'global' || scope === 'all') {
                ...
                if (ranked.length > 0) {
                    ...
                    if (ranked.length > top.length)
                        budget.add(clippedNote(ranked.length, top.length, language));
                }
                else {
