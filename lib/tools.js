...
export function memoirRecordTool(store) {
    return defineTool({
        name: 'memoir_record',
        description: '把一条「项目持久记忆」写入本项目的 PROJECT_MEMORY.md 与全局索引 ~/.dsh/dsh-memoir.json。' +
        ...
        output: {
            ...
            render: (_args, value) => text(`已记录 [${value.section}] 记忆 (id: ${value.id})\n` +
...
export function memoirReadTool(store) {
    return defineTool({
        name: 'memoir_read',
        description: '读取项目持久记忆与经验教训。开始新会话或接手旧项目时先调用，了解既有工作归纳、经验教训与行动指南。' +
