...
export function validateEntryPayload(payload) {
  ...
  if (!Object.prototype.hasOwnProperty.call(SECTIONS, payload.section)) {
    return `section must be one of ${SECTION_KEYS.join('/')}`
  }
  if (typeof payload.content !== 'string' || payload.content.trim() === '') return 'content is required'
      ...
      const entries = Array.isArray(project.entries)
        ? project.entries
            ...
            .map((e) => ({
              ...
              section: Object.prototype.hasOwnProperty.call(SECTIONS, e.section) ? e.section : 'note',
