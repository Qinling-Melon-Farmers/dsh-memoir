...
export const MEMOIR_LANGUAGES = ['zh', 'en'];
export const DEFAULT_MEMOIR_LANGUAGE = 'zh';
const zh = {
    guidance: 'dsh-memoir 提供项目持久记忆。下方仅注入本项目高优先级记忆；' +
        '需要历史细节时调用 memoir_read；产生可复用的工作结论、经验或后续行动时调用 memoir_record。',
    sectionHeading: '## 项目持久记忆（自动注入）',
    distillPrompt: '（dsh-memoir 自动收尾）本轮工作已结束，请把本轮归纳沉淀进项目记忆：\n' +
        ...
        '只保留已验证的稳定决策、可复用经验、后续行动和环境约束；保留否定条件与适用范围，不把猜测或计划写成完成事实。不要保存密钥、访问令牌或逐步工具日志。已有结论优先 memoir_update，冲突需显式替代。\n' +
        '最终回复保持一句话以内，不要展开。',
    sections: {
...
const en = {
    guidance: 'dsh-memoir provides persistent project memory. Only high-priority memory for this project is injected below; ' +
    ...
    distillPrompt: '(dsh-memoir automatic wrap-up) This work turn has ended. Distill it into project memory:\n' +
        ...
        'Keep only verified stable decisions, reusable lessons, next actions, and environment constraints. Preserve negation and scope; never turn guesses or plans into completed facts. Exclude secrets, access tokens, and step-by-step tool logs. Prefer memoir_update for existing conclusions and explicit superseding for conflicts.\n' +
    ...
    record: {
        description: 'Persist one project-memory entry for future sessions. At the end of a task, record completed work (work), reusable lessons (lessons), and follow-up actions (actions) as separate entries. ' +
...
export function languageFrom(source) {
    return resolveMemoirLanguage(typeof source === 'function' ? source() : source);
}
export function hostCopy(language) {
