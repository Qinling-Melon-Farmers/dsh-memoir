...
export const HOT_SECTION_ORDER = ['actions', 'lessons', 'work'];
...
export function renderHotMemory(selected) {
    ...
    for (const section of HOT_SECTION_ORDER) {
        ...
        if (group.length === 0)
            continue;
        const label = section === 'actions' ? 'Actions:' : section === 'lessons' ? 'Lessons:' : 'Work:';
        lines.push(label);
...
export function selectHotMemory(entries, budget = DEFAULT_MEMORY_BUDGET, now = Date.now()) {
    ...
    const selected = [];
    const measure = () => (selected.length === 0 ? '' : renderHotMemory(selected));
